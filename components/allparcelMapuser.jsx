  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Now call your API
        const res = await fetch(`/api/getGeojsonUser?user_id=${currentUserId}`);
        const geojson = await res.json();
        setGeoJsonData(geojson);
        setParcels(geojson.features || []);
        console.log("GeoJSON data:", geojson);
        console.log("Parcels:", geojson.features);
      } catch (err) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <button
        ...
        className="absolute top-4 left-4 z-[10000] bg-white border border-gray-300 rounded-full shadow p-2 hover:bg-gray-100 transition"
        style={{ display: 'block' }}
      >
        {sidebarOpen ? (
      ...
      {/* Map container, adjust width if sidebar is open */}
      <div className={`flex-1 relative transition-all duration-300 ${sidebarOpen ? 'ml-0' : ''}`} style={{ width: sidebarOpen ? 'calc(100% - 20rem)' : '100%' }}>
        <MapContainer center={[9.082, 8.6753]} zoom={6} style={{ height: "100%", width: "100%" }}>
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            attribution='&copy; <a href="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}'
          />
