...
import FeeLayout from "./calculatefeeLayout";
...
const LayoutStepForm = () => {


    const steps = ["Upload CSV", "Calculate Area",  "No of Plot", "Fee Details","Client Info", "Review"]
...
const progress = ((step - 1) / (steps.length - 1)) * 100
  ...
  const onSubmit =  (data) => {
    ...
    setIsLoading(true)
      const fetchPillar = async () => {     
        ...
        .insert([
          { user_id: userid,  area: area, pillar_type: data.pillarType,  paid_amount: data.paymentAmount, pillar_no: data.pillarNo, location: location, lga: lga,  land_use: data.landUse,
          client_address: data.clientAddress, client_name: data.clientName, plan_number: data.planNumber, status: 'pending',  geom: geojson, schedule: schedule.schedule, layout_url: fileUrl, land_use: 'Layout',  pilot_count: Number(pilot)}  
        ])
        .single();
  ...
  return (
    ...
    >
      ...
      {step === 3 && <FeeLayout  watch={watch} onPilot={onPilot} onfileUrl={onfileUrl} setValue={setValue} register={register} nextStep={nextStep} prevStep={prevStep} onSubmit={handleSubmit(onSubmit)} area={area} schedule={schedule} />}
      {step === 4 && <Step3 watch={watch} register={register} nextStep={nextStep} prevStep={prevStep} setValue={setValue}  />}
